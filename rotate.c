...
/*   Updated: 2024/11/20 23:59:19 by frubio-i         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate(t_list **lst)
{
	...
	tail = *lst; // Tail apunta a primer nodo para recorrerlo e ir al ultimo
	tail = list_last(*lst); // Recorremos hasta el ultimo nodo (funcion)
	tail->next = head; // Apuntamos el ultimo al primer nodo (Head)
	*lst = head->next; // Apuntamos a segundo nodo para que sea el primero
	head->next = NULL; // El primer nodo (ahora ultimo), apunta a NULL
}
